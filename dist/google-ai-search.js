...
export class GoogleAISearchExtractor {
    ...
    async searchAIContent(query, options = {}) {
        try {
...
export async function getGoogleAISearchResponses(queries, options = {}) {
    const extractors = queries.map(() => new GoogleAISearchExtractor());
    try {
        const results = await Promise.all(queries.map((query, index) => extractors[index].searchAIContent(query, options)));
        return results;
    }
    finally {
...
export async function getGoogleAISearchResponse(query, options = {}) {
